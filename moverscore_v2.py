...
def process(a):
    a = ["[CLS]"]+tokenizer.tokenize(a,max_length=384,truncation=True)+["[SEP]"]
...
def get_bert_embedding(all_sens, model, tokenizer, idf_dict,
    ...
    padded_sens, padded_idf, lens, mask, tokens = collate_idf(all_sens,
                                                      lambda x: tokenizer.tokenize(x, max_length=512, truncation=True),tokenizer.convert_tokens_to_ids,idf_dict,device=device)
